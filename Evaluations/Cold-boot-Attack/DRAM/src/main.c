...
int main(int argc, char **argv)
{
   ...
   /* mem = open("/dev/uio0", O_RDWR);
	...
	int bmp_fd, vul_mem = 0x3cbe0000;
	bmp_fd = open("/dev/mem",  O_RDWR);
	bmp = mmap(NULL, sizeof(bmp_mem), PROT_READ | PROT_WRITE, MAP_SHARED, bmp_fd, vul_mem);

	if (bmp == MAP_FAILED)
	{
		mp_printf("BMP MMAP Failed! Error = %d\r\n", errno);
	   return -1;
	}
	else
	{
		mp_printf("BMP MMAP Success\r\n");
	}
    //   while (1)
     //  {
   		//void *p = (void *) 0x41200000;
	printf("pointer is at address %p value(physical address) %p \n",&bmp, bmp);
	printf("Before:\r\n");
	for (int i = 0; i < 64; i++)
	{
		printf("%x ", *( bmp->bmp+ i));
	...
	fwrite(bmp->bmp, 1, sbT.st_size ,fp);
	fclose(fp);
	printf("\nDone writing:\n");
	// sleep(5);
	load_file("1", bmp->bmp);
